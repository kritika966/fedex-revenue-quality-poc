...
#          the DIM fix alone won't cure. This is a PRACTITIONER HEURISTIC,
#          not a derived profit-maximizing price — it does not solve the
#          textbook monopoly-pricing first-order condition (that would
#          set markup ~ -1/elasticity with no cap). Instead it:
#            1. scales the ask inversely with elasticity magnitude via an
#               arbitrary constant (1.5), so more price-sensitive accounts
#               get a smaller ask than less price-sensitive ones, then
#            2. clips the result to a pragmatic 2-9% band, because real
#               pricing teams cap increases for relationship/contract
#               reasons regardless of what a formula says.
#          A true optimum would require a fitted (not assumed) elasticity
#          and a cost baseline to solve against — out of scope for this
#          POC. The NET revenue effect below is computed honestly against
#          this heuristic price, not against a theoretical optimum.
def recommend_price_action(row):
    ...
    # heuristic ask: never propose more than 9%, never less than 2%, scale
    # inversely with elasticity magnitude (more elastic accounts get
    # smaller asks) — see module-level note above; this is NOT a solved
    # optimum, the "1.5" is a chosen constant, not a fitted parameter.
